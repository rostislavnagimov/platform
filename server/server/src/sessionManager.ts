...
export class TSessionManager implements SessionManager {
  ...
  createSession (
    ...
    return new ClientSession(ctx, token, ws, workspaces, account, info, token.extra?.mode === 'backup')
  }
  ...
  async addSession (
    ...
    this.accountIdToSessions.set(account.account, existingSessions.concat(session))

    for (const sid of session.getUserSocialIds()) {
      this.socialStringsToUsers.set(sid, account.account)
    }
    ...
    for (const workspaceRef of session.workspaces) {
      ...
      const info = account.workspaces[workspaceRef.wsIds.uuid]
      if (info === undefined) {
        continue
      }
      const workspace = this.getWorkspace(ctx, workspaceRef.wsIds.uuid, info)
      if (workspace !== undefined) {
        ...
        if (workspace !== undefined) {
          ...
          this.tickHandlers.set(sessionRef.sessionId, {
            ...
            operation: async () => {
              ...
              const allSessions = (this.accountIdToSessions.get(account) ?? []).filter(
                (it) =>
                  it.sessionId !== sessionRef.sessionId &&
                  !it.workspaces.some((it) => it.wsIds.uuid === workspace.wsId.uuid)
              )
              if (allSessions.length === 0) {
